...
9. **Async DB operations**: All repository methods that perform any EF Core database operation (`SaveChanges`, `ToList`, `FirstOrDefault`, `Find`, `Any`, `Count`, etc.) must use their async counterparts (`SaveChangesAsync`, `ToListAsync`, `FirstOrDefaultAsync`, `FindAsync`, `AnyAsync`, `CountAsync`, etc.) and return `Task<T>` or `Task`. Controller actions that call async repository methods must be `async Task<IActionResult>`. The only exception is computed/in-memory-only methods that do not touch the database at all. Push all filtering to SQL — avoid intermediate `.ToList()` calls mid-query that pull data to memory before filtering, unless there is a documented reason the expression cannot be translated to SQL by EF Core.
10. **Hardcoded strings**: Never leave hardcoded user-facing strings inline in C# code. Place them in the appropriate static constants class instead:
   - **Error and validation messages** (thrown exceptions, model validation messages) → `Models/ErrorMessage.cs` (`ErrorMessage` class)
   - **Display labels, fallback names, UI copy used in C# code** → `Models/DisplayConstant.cs` (`DisplayConstant` class)
   - Razor view strings that are purely presentational and not referenced from C# do not need to be extracted.